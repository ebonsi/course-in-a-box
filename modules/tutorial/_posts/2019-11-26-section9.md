...
#### Since you are there with the terminal open, let us create the ```httpd-userdir.conf``` file!


```[admin:~] root# nano /usr/local/apac/Apache2/conf/extra/httpd-userdir.conf``` 


```press return```

#### Inside of this file you will write this code,
...
#
Include /usr/local/apac/apache2/conf/users/*.conf
...
#### to exit and go back to root.

#### Next we will be editing the ```httpd.conf``` file located inside of the ```Apache2/conf``` directory. This is a big file, so you will have to take your time and follow it's configuration step-by-step. Remember that you are getting this easy! Many times it takes hours to configure this files until it work ok without any errors. So do not take that for granted!
...
# least PidFile.
#
ServerRoot "/usr/local/apac/apache2"
...
<IfModule alias_module>
    ...
    #
    ScriptAlias /cgi-bin/ "/usr/local/apac/apache2/cgi-bin/"
...
#
# "/usr/local/apps/apache2/cgi-bin" should be changed to whatever your ScriptAliased
...
#Include conf/extra/srs/modsecurity_crs_10_setup.conf
#Include conf/extra/srs/base_rules/*.conf
</IfModule>
#
...
# Configuring HSTS on Apache | Redirecting HTTP connections to HTTPS
...
Include "/usr/local/apac/apache2/conf/ssi.conf"
AddType application/x-httpd-php .php
    <IfModule php7_module>
            PHPIniDir "/usr/local/apac/php/etc"
    </IfModule>

# OSX Users directories
Include /usr/local/apac/apache2/conf/users/*.conf

Include "conf/deflate.conf"
# Include "/usr/local/apac/apache2/conf/bitnami/bitnami.conf"

###end of file

````

#### Next we will be editing the ```httpd-ssl.conf```!

# Stop! Warning: From this point on, you must have a domain registred with a Registrar or Domain host!

#### The reason is that you are going to configure your SSL Certificate of authority files and get a Certificate of Authority from Letsencrypt.org and if you do not have an FQDN, ```Full Qualified Domain Name```, you will run into errors and Apache will not properly start. So, make sure you have your FQDN and insert it whenever you see the words, "domain1.com" in the Apache configuration files.
